...
// 30 M * 12 * 4 = 1.5G
#define GLOBLA_MODLE_VERTEX_NUM (1024 * 1024 * 60)
...
//        boost::unique_lock<boost::mutex> lock(update_globalMap_mutex);
    ...
    void inter_all(std::vector<Frame> &frame_list, const std::vector<MultiViewGeometry::KeyFrameDatabase> &kflist);

    inline void global_optimization()
    {
        while(1)
        {
            boost::unique_lock<boost::mutex> lock(mutex_global_opti);
            mutex_global_opti_condi.wait(lock);

            //也就是最后一帧的index
            count_global_opti=gcSLAM.globalFrameList.size();
            cout<<"global optimization start"<<endl;
            double time1= (double)cv::getTickCount();
            optimizeKeyFrameMapRobust_global( gcSLAM.keyFrameCorrList, gcSLAM.globalFrameList,gcSLAM.KeyframeDataList,0, 0.5);    

            double time2= (double)cv::getTickCount() - time1;
            time2=time2*1000/ cv::getTickFrequency();
            cout<<"global optimization duration time: "<<time2<<endl;
        }
...
//            printf("integrating frame: %d %d\r\n",local_frame.frame_index, integrateFlag);
    ...
    void initChiselMap(const MultiViewGeometry::CameraPara &camera, float ipnutVoxelResolution, float farPlaneDist = 3)
    {
        ...
        std::cout << "far plane distance: " << farPlaneDist <<"m"<<std::endl;
        chunkSizeX=  8;
        chunkSizeY=  8;
        chunkSizeZ=  8;
