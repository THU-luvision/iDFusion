...
namespace MILD
{
        ...
        void search(cv::Mat desc, std::vector<cv::DMatch> &matches)
        {
            ...
            for (unsigned short feature_idx = 0; feature_idx < feature_num; feature_idx++)
            {
                ...
                for (int hash_table_id = 0; hash_table_id < hash_table_num; hash_table_id++)
                {
                    ...
                    {
                        unsigned int feature_index = features_buffer[entry_idx][i]; // 原有帧的feature_index
                        uint64_t * f2 = features_descriptor + feature_index * 4;    // 找到对应的特征点描述子 256bit
                ...
                cv::DMatch m;
                m.queryIdx = feature_idx;       // 查询的帧的feature_index
                m.trainIdx = best_corr_fid;     // 匹配的fezture_index
                m.distance = min_distance;      // 上面两个index的distance
                matches.push_back(m);
            }
        }
                             ...
                             int hamming_distance_threshold)
    {
        ...
        int calculate_hamming_distance_256bit(uint64_t * f_1, uint64_t * f_2)
        {
            ...
            // 计算汉明距离,_mm_popcnt_u64返回比特为1的位的个数 ^相同的比特置1， 下面的加法是处理了256位，也就是一个描述子的长度
    ...
    private:
		...
		unsigned int hash_table_num;				// basic parameters of MIH, hash table num   hash_table_num×bits_per_substring=descriptor_length
		unsigned int depth_level;					// depth of MIH
		unsigned int entry_num_per_hash_table;		// entry num per hash table
		unsigned int buckets_num;					// total entry num      buckets_num=entry_num_per_hash_table×hash_table_num
